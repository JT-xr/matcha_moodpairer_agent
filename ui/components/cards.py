...
def render_cafe_card(cafe_data, card_index=0):
    ...
    <div style="{get_cafe_card_style()}">
        <h4 style="color: #2c3e50; margin: 0 0 10px 0; font-size: 20px;">
            {cafe_data['name']} 
            <span style="color: #ff9800;">⭐ {cafe_data['rating']}</span>
        </h4>
    ...
    with col_a:
        if st.button("🗺️ Directions", key=f"directions_{card_index}", use_container_width=True):
            st.success(f"Opening directions to {cafe_data['name']}...")
    with col_b:
        if st.button("📞 Call", key=f"call_{card_index}", use_container_width=True):
            st.info(f"Calling {cafe_data['phone']}...")
    with col_c:
        if st.button("💾 Save", key=f"save_{card_index}", use_container_width=True):
            st.success(f"Saved {cafe_data['name']} to favorites!")
